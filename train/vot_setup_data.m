...
switch opts.version%vot15:21395 vot14:10188
    case 1,
        ...
        imdb.images.bbox = zeros(1,1,4,numel(set),'single');
    case 2,
        set_name = {'nus_pro'};
...
if any(strcmpi(set_name,'vot15'))
    ...
    parfor  v = 1:numel(videos)
        ...
        for frame = 1:(numel(im_bank)-1)
            video_frame_expDir = [video_expDir '/' num2str(frame) '-%d' ];
    ...
    for  v = 1:numel(videos)
        ...
        for frame = 1:(numel(img_files)-1)
            video_frame_expDir = [video_expDir '/' num2str(frame) '-%d' ];
            load([sprintf(video_frame_expDir,0),'.mat']);
            imdb.images.bbox(:,:,:,now_index+(1:nsample)) = bbox_gt_scaled;
            for i = 1:nsample
                imdb.images.target(now_index+i) = {[sprintf(video_frame_expDir,0),'.jpg']};
                imdb.images.image(now_index+i) = {[sprintf(video_frame_expDir,i),'.jpg']};
            end
