...
opts = [];
opts.dataDir = '../data';
opts.version = 1;    % 1 vot 2 vot-lite 3 det 4 full
opts = vl_argparse(opts, varargin) ;
...
switch opts.version%vot15:21395 vot14:10188
    case 1,
        ...
        imdb.images.set = set;
        imdb.images.target = zeros(227,227,3,numel(set),'single');%TODO:use name
        imdb.images.search = zeros(227,227,3,numel(set),'single');%TODO:use name
...
if any(strcmpi(set_name,'vot15'))
    ...
    disp('VOT2015 Data(for Validation):');
    ...
    for  v = 1:numel(videos)
        ...
        ground_truth_2xy = get_axis_aligned_BB(ground_truth_4xy);
        raw_img_bank = vl_imreadjpeg(img_files);
        
        for frame = 1:(numel(img_files)-1)
            if(lite_index(now_index))
                [search_2xy_in_pad,pad_target,pad_search] = ...
                    crop_pair_pad_image(...
                    raw_img_bank{frame},raw_img_bank{frame+1},...
                    ground_truth_2xy(frame,:),ground_truth_2xy(frame+1,:));
                
                imdb.images.target(:,:,:,lite_index(now_index)) = pad_target;
                imdb.images.search(:,:,:,lite_index(now_index)) = pad_search;
                imdb.images.bboxs(1,1,1:4,lite_index(now_index)) =...
                    single(search_2xy_in_pad) ;
            end
