...
%         error('miss_matching');
%     end
%     ILSVRC_val_frame_num(i) = numel(img_files);
%     if visual,
%         close all
%         update_visualization = show_video(img_files);
%         for frame = 1:numel(img_files),
...
if numel(img_files)~=size(ground_truth_4xy,1)
    error('miss_matching');
end
ILSVRC_det_frame_num = numel(img_files);
if visual,
    close all
    update_visualization = show_video(img_files);
    for frame = 1:numel(img_files),
        stop = update_visualization(frame, ground_truth_4xy(frame,:),[],[]);
        if stop, break, end
        drawnow;
    end
end
